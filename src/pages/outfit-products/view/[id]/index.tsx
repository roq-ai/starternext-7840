...
import { getOutfitProductById } from 'apiSdk/outfit-products';
import { OutfitProductInterface } from 'interfaces/outfit-product';

function OutfitProductViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('outfit_product', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/outfit-products/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('outfit', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Outfit"
                    text={
                      <Link as={NextLink} href={`/outfits/view/${data?.outfit?.id}`}>
                        {data?.outfit?.name}
                      </Link>
                    }
                  />
