...
import { OutfitProductListPage as ShowCaseList } from 'pages/outfit-products';

function DashboardPage() {
  const { session } = useSession();

  const outfitStat = useSWR<PaginatedInterface<OutfitInterface>>(
    () => `/outfits`,
    () => getOutfits({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Outfits',
      result: outfitStat?.data?.totalCount,
      isLoading: outfitStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Outfit Products
                </Text>
                <NextLink href={`outfit-products`} passHref legacyBehavior>
                  <Button
