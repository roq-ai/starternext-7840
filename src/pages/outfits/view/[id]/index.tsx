...
import { getOutfitById } from 'apiSdk/outfits';
import { OutfitInterface } from 'interfaces/outfit';
import { OutfitProductListPage } from 'pages/outfit-products';

function OutfitViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<OutfitInterface>(
    () => (id ? `/outfits/${id}` : null),
    () =>
      getOutfitById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('outfit', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/outfits/edit/${id}`} passHref legacyBehavior>
                    <Button
